              ...
              <Card className="relative z-10 bg-card text-card-foreground shadow-md transition-shadow duration-300 hover:shadow-lg sm:static sm:z-auto">
                <CardContent className="flex flex-col items-start p-4">
                  ...
                  <p className="mb-3 text-sm text-muted-foreground">
                    {res.repository_description}
                  </p>
                  <div className="w-full">
                    {res.commit_message.map((commit, commitIndex) => (
                      <div
                        key={commitIndex}
                  ...
                  <div className="mt-4 flex items-center text-xs text-muted-foreground">
                    <Avatar className="mr-2 h-6 w-6">
                      <AvatarImage src={res.author_avatar_url} />
                      <AvatarFallback>{res.author_name}</AvatarFallback>
                    </Avatar>
                    <span>Updated on {res.last_updated}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
